...
function Header({ mode }) {
    ...
    return (
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mb: 3 }}>
            ...
            <Typography variant="h5" fontWeight={700}>Investment Tracker</Typography>
            <Typography variant="body2" color="text.secondary" mt={0.5}>{subtitles[mode]}</Typography>
        </Box>
    );
}
    ...
    return (
        <Box component="form" onSubmit={onSubmit} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <Button type="submit" variant="contained" fullWidth size="large" disabled={loading}
                sx={{ py: 1.5, background: "linear-gradient(90deg, #740f87, #2421b7)", "&:hover": { background: "linear-gradient(90deg, #8a1aa0, #3530d4)" } }}>
                {loading ? <CircularProgress size={22} color="inherit" /> : "Sign in"}
            </Button>
            <Divider />
            <Box sx={{ textAlign: "center" }}>
                <Typography variant="body2" color="text.secondary" component="span">Don't have an account? </Typography>
                <Link component="button" type="button" variant="body2" fontWeight={600} onClick={() => onSwitch("signup")}>
                    Sign up
                </Link>
            </Box>
        </Box>
...
function SignUpForm({ onSwitch }) {
    ...
    return (
        <Box component="form" onSubmit={onSubmit} sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {error && <Alert severity="error">{error}</Alert>}
            <TextField label="Name" type="text" value={displayName} onChange={e => setDisplayName(e.target.value)}
                fullWidth autoComplete="name" autoFocus />
            <TextField label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)}
            ...
            <Divider />
            <Box sx={{ textAlign: "center" }}>
                <Typography variant="body2" color="text.secondary" component="span">Already have an account? </Typography>
                <Link component="button" type="button" variant="body2" fontWeight={600} onClick={() => onSwitch("signin")}>
                    Sign in
                </Link>
            </Box>
        </Box>
        ...
        return (
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <Alert severity="success">
                    ...
                    <Link href="http://localhost:4000" target="_blank" rel="noopener">localhost:4000</Link> locally).
                </Alert>
                <Button variant="outlined" fullWidth onClick={() => onSwitch("signin")}>Back to sign in</Button>
            </Box>
        );
    }
