...
import { fetchCoinData, fetchSimplePrice, fetchHistoricalPrices } from '../services/coingecko';
...
async function createHolding(coinId: string, accountId: string): Promise<string> {
    ...
    try {
        await client.query(
            ...
            [tickerId, coinData.name, coinData.symbol, coinData.currentPrice, coinData.twentyFourHourChange, coinData.marketCap, coinData.volume, coinData.imageUrl, coinId]
        );
        await client.query(
    ...
    try {
        await priceClient.query(
            ...
            [uuidv4(), tickerId, dateStr, price.gbp, price.gbp_24h_change, price.gbp_market_cap, price.gbp_24h_vol, dateStr]
        );
    } finally {
        priceClient.release();
    }
    ...
    const tsvRows = historical.prices.map((curr, i) => [
        uuidv4(), tickerId, new Date(curr[0]).toISOString(), String(curr[1]),
        'null', String(historical.market_caps[i][1]), String(historical.total_volumes[i][1]), 'null',
    ]);
...
async function createTransaction(
    holdingId: string, datetime: string, buySell: string, units: string, price: string
): Promise<void> {
    const client = await pool.connect();
    try {
        await client.query(
    ...
    app.get<{ Querystring: { accountId: string } }>('/portfolio', async (req, reply) => {
        ...
        try {
            const holdings = await client.query(
                `SELECT * FROM get_holding_view WHERE account_id = $1`,
                [accountId]
            );
